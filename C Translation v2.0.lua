...
dialog_config={
    ...
    {class="label",label="index",x=7,y=2},
    {class="intedit",name="fscx_index",value=1,x=8,y=2},
    ...
    {class="label",label="index",x=7,y=3},
    {class="intedit",name="fscy_index",value=1,x=8,y=3},

    {class="checkbox",name="frz",label="frz",value=false,x=0,y=4},
    {class="label",label="frz_start",x=1,y=4},
    {class="floatedit",name="frz_start",value=0,x=2,y=4},
    {class="label",label="frz_end",x=3,y=4},
    {class="floatedit",name="frz_end",value=0,x=4,y=4},
    {class="label",label="accel",x=5,y=4},
    {class="floatedit",name="frz_accel",value=1,x=6,y=4},
    {class="label",label="index",x=7,y=4},
    {class="intedit",name="frz_index",value=1,x=8,y=4},

    {class="checkbox",name="clip_x",label="clip_x",value=false,x=0,y=5},
    {class="label",label="clip_x_start",x=1,y=5},
    {class="floatedit",name="clip_x_start",value=0,x=2,y=5},
    {class="label",label="clip_x_end",x=3,y=5},
    {class="floatedit",name="clip_x_end",value=0,x=4,y=5},
    {class="label",label="accel",x=5,y=5},
    {class="floatedit",name="clip_x_accel",value=1,x=6,y=5},

    {class="checkbox",name="clip_y",label="clip_y",value=false,x=0,y=6},
    {class="label",label="clip_y_start",x=1,y=6},
    {class="floatedit",name="clip_y_start",value=0,x=2,y=6},
    {class="label",label="clip_y_end",x=3,y=6},
    {class="floatedit",name="clip_y_end",value=0,x=4,y=6},
    {class="label",label="accel",x=5,y=6},
    {class="floatedit",name="clip_y_accel",value=1,x=6,y=6},

    {class="checkbox",name="other_button",label="other tags",value=false,x=0,y=7},
    {class="dropdown",name="other",items={"fsp","fsvp","fax","fay","frx","fry"},x=0,y=8},
    {class="label",label="other_start",x=1,y=8},
    {class="floatedit",name="other_start",value=0,x=2,y=8},
    {class="label",label="other_end",x=3,y=8},
    {class="floatedit",name="other_end",value=0,x=4,y=8},
    {class="label",label="accel",x=5,y=8},
    {class="floatedit",name="other_accel",value=1,x=6,y=8},
    {class="label",label="index",x=7,y=8},
    {class="intedit",name="other_index",value=1,x=8,y=8},

    {class="checkbox",name="set",label="setting",value=true,x=0,y=9,hint="true: time mode, false: line mode"},
    --note
    {class="label",x=0,y=10,width=9,label="Translation v2.0"},
    {class="label",x=0,y=11,width=9,label="better to be used in FRAME BY FRAME lines, may use the linetofbf in Relocator first, accel argument (0,inf), index argument Z+"},
    {class="label",x=0,y=12,width=9,label="index: index of the tag you would like to translate in ALL this tag"},
    {class="label",x=0,y=13,width=9,label="ATTENTION: positive posy means moving downwards"}
}
buttons={"Run","Quit"}
        ...
        for si,li in ipairs(selected) do
            ...
            if (result["posx"]==true or result["posy"]==true) then
                ...
                if (result["posx"]==true) then
                    ...
                    linetext=linetext:gsub("\\pos%([^,]*,",string.format("\\pos(%.3f,", gposx + interpolate(result["posx_start"],result["posx_end"],result["posx_accel"],N,T,i,t,result["set"])))
                end
                if (result["posy"]==true) then
                    local gpx, gpy = linetext:match("\\pos%(([^,]*),([^%)]*)%)")
                    gpy = tonumber(gpy)+interpolate(result["posy_start"],result["posy_end"],result["posy_accel"],N,T,i,t,result["set"])
            ...
            if (result["fscx"]==true) then
                ...
                deviation = interpolate(result["fscx_start"],result["fscx_end"],result["fscx_accel"],N,T,i,t,result["set"])
            ...
            if (result["fscy"]==true) then
                ...
                deviation = interpolate(result["fscy_start"],result["fscy_end"],result["fscy_accel"],N,T,i,t,result["set"])
            ...
            if (result["frz"]==true) then
                ...
                deviation = interpolate(result["frz_start"],result["frz_end"],result["frz_accel"],N,T,i,t,result["set"])
                    ...
                    function(c,d) 
                        ...
                        for head,num in d:gmatch("([^%d%.%-]+)([%d%.%-]+)") do
                            if (o_e == 0 and result["clip_x"]==true) then 
                                trs_clip = string.format("%s%s%.2f",trs_clip,head,num+interpolate(result["clip_x_start"],result["clip_x_end"],result["clip_x_accel"],N,T,i,t,result["set"]))
                            elseif (o_e == 1 and result["clip_y"]==true) then
                                trs_clip = string.format("%s%s%.2f",trs_clip,head,num+interpolate(result["clip_y_start"],result["clip_y_end"],result["clip_y_accel"],N,T,i,t,result["set"]))
                            else
            ...
            if (result["other_button"]==true) then
                ...
                if (result["other"]=="fsp") then
                    ...
                    deviation = interpolate(result["other_start"],result["other_end"],result["other_accel"],N,T,i,t,result["set"])
...
function interpolate(head,tail,accel,N,T,i,t,judge)
    -- i 1-N
    if (judge==false) then
